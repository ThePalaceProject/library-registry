...
VENV_BASE_DIR = Path("/libreg_venv")
VENV_ACTUAL = [Path(VENV_BASE_DIR / d) for d
               in os.listdir(VENV_BASE_DIR)
               if d.startswith("libreg_app-") and os.path.isdir(VENV_BASE_DIR / d)][0]
...
chdir = os.environ.get("LIBREG_HOME", "/libreg_app") # change to this dir before loading apps
