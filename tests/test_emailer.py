...
class TestEmailer(DatabaseTest):
    ...
    def test_from_sitewide_integration(self):
        ...
        integration = self._integration()
        mock_url_for = object()
        emailer = Emailer.from_sitewide_integration(self._db, mock_url_for)
        ...
        eq_(mock_url_for, emailer.url_for)
        ...
        emailer = Emailer.from_sitewide_integration(self._db, None)
        for email_type in Emailer.EMAIL_TYPES:
            template = emailer.templates[email_type]
    ...
    def test_constructor(self):
        ...
        mock_templates = object()
        args['templates'] = mock_templates
        mock_url_for = object()
        args['url_for'] = mock_url_for

        m = Emailer
        assert_raises_regexp(
            CannotLoadConfiguration, "No SMTP username specified", **args
        )
        args['smtp_username'] = 'user'

        assert_raises_regexp(
            CannotLoadConfiguration, "No SMTP password specified", **args
        )
        args['smtp_password'] = 'password'

        assert_raises_regexp(
            CannotLoadConfiguration, "No SMTP host specified", **args
        )
        args['smtp_host'] = 'host'
...
If the link expires, just re-register your library with the library registry, and a fresh validation email like this will be sent out."""
    ...
    def test_send(self):
        ...
        emailer = MockEmailer.from_sitewide_integration(self._db, None)
