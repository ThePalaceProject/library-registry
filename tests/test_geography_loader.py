...
class TestGeographyLoader(DatabaseTest):
    ...
    def test_load(self):       
        ...
        texas_zip = self.loader.load(metadata, geography)
        eq_("77977", texas_zip.external_id)
        eq_("77977", texas_zip.name)
        eq_(None, texas_zip.full_name)
        ...
        new_york = self.loader.load(metadata, geography)
        eq_("NY", new_york.abbreviated_name)
        eq_("New York", new_york.full_name)
...
{"type": "Point", "coordinates": [-86.034128, 32.302979]}"""
        ...
        [us, alabama, montgomery] = list(self.loader.load_ndjson(input))
