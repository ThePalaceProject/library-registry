...
def loader(db_session):
    loader = GeometryLoader(db_session)
    yield loader


class TestGeometryLoader:
    @pytest.mark.needsdocstring
    def test_load(self, db_session, loader):
        """
        GIVEN:
        WHEN:
        THEN:
        ...
        geography = (
            ...
            '[[[-96.840066, 28.683039], [-96.830637, 28.690131], [-96.835048, 28.693599], '
            '[-96.833515, 28.694926], [-96.82657, 28.699584], [-96.822495, 28.695826], [-96.821248, 28.696391], '
            '[-96.814249, 28.700983], [-96.772337, 28.722765], [-96.768804, 28.725363], [-96.768564, 28.725046], '
            '[-96.767246, 28.723276], [-96.765295, 28.722084], [-96.764568, 28.720456], [-96.76254, 28.718483], '
            '[-96.763087, 28.717521], [-96.761814, 28.716488], [-96.761088, 28.713623], [-96.762231, 28.712798], '
            '[-96.75967, 28.709812], [-96.781093, 28.677548], [-96.784803, 28.675363], [-96.793788, 28.669546], '
            '[-96.791527, 28.667603], [-96.808567, 28.678507], [-96.81505, 28.682946], [-96.820191, 28.684517], '
            '[-96.827178, 28.679867], [-96.828626, 28.681719], [-96.831309, 28.680451], [-96.83565, 28.677724], '
            '[-96.840066, 28.683039]]]}'
        )
        ...
        metadata = (
            '{"parent_id": null, "name": "New York", "type": "state", "abbreviated_name": "NY", '
            '"id": "NY", "full_name": "New York", "aliases": [{"name": "New York State", "language": "eng"}]}'
        )
        ...
        for place_alias_obj in db_session.query(PlaceAlias).all():
            db_session.delete(place_alias_obj)
    ...
    @pytest.mark.needsdocstring
    def test_load_ndjson(self, db_session, loader):
        """
        GIVEN:
        WHEN:
        THEN:
        ...
        (old_us, _) = get_one_or_create(db_session, Place, parent=None, external_name="United States",
        ...
        test_ndjson_lines = [
            '{"parent_id": null, "name": "United States", "aliases": [{"name" : "The Good Old U. S. of A.", "language": "eng"}], "type": "nation", "abbreviated_name": "US", "id": "US"}',  # noqa: E501
            '{"type": "Point", "coordinates": [-159.459551, 54.948652]}',
            '{"parent_id": "US", "name": "Alabama", "aliases": [], "type": "state", "abbreviated_name": "AL", "id": "01"}',     # noqa: E501
            '{"type": "Point", "coordinates": [-88.053375, 30.506987]}',
            '{"parent_id": "01", "name": "Montgomery", "aliases": [], "type": "city", "abbreviated_name": null, "id": "0151000"}',  # noqa: E501
            '{"type": "Point", "coordinates": [-86.034128, 32.302979]}',
        ]
        input = StringIO("\n".join(test_ndjson_lines))
        [(us, _), (alabama, _), (montgomery, _)] = list(loader.load_ndjson(input))

        # All three places were loaded as Place objects and their relationships to each other were maintained.
        ...
        # The place that existed before we ran the loader is still the same database object, but it has
        # had additional information associated with it.
        assert old_us == us
