...
class TestPlace(DatabaseTest):
    ...
    def test_creation(self):
        ...
        places = self._db.query(Place).filter(Place.type == Place.STATE).order_by(distance).add_columns(distance)

        # We can find the distance in kilometers between the 'Lake
        # Placid' point and the points representing the other states.
        expected = [("New York", 172), ("Connecticut", 285), ("New Mexico", 2993)]
        actual = [(x[0].external_name, int(x[1]/1000)) for x in places]
    ...
    def test_default_nation(self):
        ...
        setting = ConfigurationSetting.sitewide(self._db, Configuration.DEFAULT_NATION_ABBREVIATION)
