...
class TestVendorIDRequestParsers:
    ...
    def test_username_sign_in_request(self):
        ...
        expected = {'username': 'Vendor username', 'password': 'Vendor password', 'method': 'standard'}
...
class TestVendorIDRequestHandler:
    ...
    def test_handle_username_sign_in_request_success(self):
        doc = self.username_sign_in_request % {"username": "user1", "password": "pass1"}
        ...
        expected_start = (
            ...
            'Human-readable label for user1</label>\n</signInResponse>'
        )
        assert result.startswith(expected_start)

    def test_handle_username_sign_in_request_failure(self):
        doc = self.username_sign_in_request % {"username": "user1", "password": "wrongpass"}
        result = self._handler.handle_signin_request(doc, self._standard_login, self._authdata_login)
    ...
    def test_handle_username_authdata_request_success(self):
        doc = self.authdata_sign_in_request % {"authdata": base64.b64encode("The secret token")}
        result = self._handler.handle_signin_request(doc, self._standard_login, self._authdata_login)
        expected_start = (
            '<signInResponse xmlns="http://ns.adobe.com/adept">\n<user>test-uuid</user>\n<label>'
            'Human-readable label for user1</label>\n</signInResponse>'
    ...
    def test_handle_username_authdata_request_invalid(self):
        doc = self.authdata_sign_in_request % {"authdata": "incorrect"}
        result = self._handler.handle_signin_request(doc, self._standard_login, self._authdata_login)
    ...
    def test_handle_username_authdata_request_failure(self):
        doc = self.authdata_sign_in_request % {"authdata": base64.b64encode("incorrect")}
        result = self._handler.handle_signin_request(doc, self._standard_login, self._authdata_login)
    ...
    def test_failure_send_accountinfo_request_to_login(self):
        doc = self.accountinfo_request % {"uuid": self.user1_uuid}
        result = self._handler.handle_signin_request(doc, self._standard_login, self._authdata_login)
        expected = (
    ...
    def test_handle_accountinfo_failure(self):
        doc = self.accountinfo_request % {"uuid": "not the uuid"}
        result = self._handler.handle_accountinfo_request(doc, self._userinfo)
        expected = (
            '<error xmlns="http://ns.adobe.com/adept" data="E_1045_ACCOUNT_INFO '
            'Could not identify patron from \'not the uuid\'."/>'
        )
        assert result == expected
