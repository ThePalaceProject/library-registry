...
class IntegrationException(Exception):
    ...
    This may be because communication failed (RemoteIntegrationException), or because local
    configuration is missing or obviously wrong (CannotLoadConfiguration).
    """
    ...
    def __init__(self, message, debug_message=None):
        ...
        :param message: The normal message passed to any Exception constructor.

        :param debug_message: An extra human-readable explanation of the problem, shown to admins but not
        to patrons. This may include instructions on what bits of the integration configuration might need
        to be changed.

        For example, an API key might be wrong, or the API key might be correct but the API provider might
        not have granted that key enough permissions.
        """
...
class RemoteIntegrationException(IntegrationException):
    ...
    def __init__(self, url_or_service, message, debug_message=None):
        ...
        `param url_or_service` The name of the service that failed (e.g. "Overdrive"), or the specific URL
...
class BadResponseException(RemoteIntegrationException):
    ...
    def from_response(cls, url, message, response):
        ...
        if isinstance(response, tuple):
            # The response has been unrolled into a (status_code, headers, body) 3-tuple.
        ...
        return BadResponseException(url, message, status_code=status_code, debug_message=msg)
    ...
    def bad_status_code(cls, url, response):
        ...
        return cls.from_response(url, message, response)


class RequestNetworkException(RemoteIntegrationException, requests.exceptions.RequestException):
    """
...
class HTTP:
    ...
    def request_with_timeout(cls, http_method, url, *args, **kwargs):
        ...
        return cls._request_with_timeout(url, requests.request, http_method, *args, **kwargs)
    ...
    def debuggable_request(cls, http_method, url, make_request_with=None,
                           **kwargs):
        """
        Make a request that returns a detailed problem detail document on error, rather than a generic "an integration
        error occured" message.

        :param http_method: HTTP method to use when making the request.
        :param url: Make the request to this URL.
        :param make_request_with: A function that actually makes the HTTP request.
        :param kwargs: Keyword arguments for the make_request_with function.
        """
        logging.info("Making debuggable %s request to %s: kwargs %r", http_method, url, kwargs)
        make_request_with = make_request_with or requests.request
        return cls._request_with_timeout(url, make_request_with, http_method,
                                         process_response_with=cls.process_debuggable_response, **kwargs)

    @classmethod
    def process_debuggable_response(cls, url, response, disallowed_response_codes=None, allowed_response_codes=None):
        """
        If there was a problem with an integration request, return an appropriate ProblemDetail. Otherwise,
        ...
        if str(code) in allowed_response_codes or series in allowed_response_codes:
            # Whether or not it looks like there's been a problem, we've been told to let this response code through.
            return response

        content_type = response.headers.get('Content-Type')
        if content_type == PROBLEM_DETAIL_JSON_MEDIA_TYPE:
            # The server returned a problem detail document. Wrap it in a new document that represents the integration
            # failure.
            problem = INTEGRATION_ERROR.detailed(
                lgt('Remote service returned a problem detail document: %r') % (response.content)
            )
        ...
        return INTEGRATION_ERROR.detailed(
            lgt("%s response from integration server: %r") % (response.status_code, response.content)
        )
    ...
    def _request_with_timeout(cls, url, make_request_with, *args, **kwargs):
        """
        Call some kind of method and turn a timeout into a RequestTimedOut exception.
        ...
        """
        process_response_with = kwargs.pop('process_response_with', cls._process_response)
        ...
        try:
            if verbose:
                logging.info("Sending request to %s: kwargs %r", url, kwargs)

            if len(args) == 1:
                # requests.request takes two positional arguments, an HTTP method and a URL. In most cases,
                # the URL gets added on here. But if you do pass in both arguments, it will still work.
                args = args + (url,)

            response = make_request_with(*args, **kwargs)

            if verbose:
                logging.info(f"Response from {url}: {response.status_code} {response.headers} {response.content}")

        except requests.exceptions.Timeout as e:
            # Wrap the requests-specific Timeout exception in a generic RequestTimedOut exception.
            raise RequestTimedOut(url, str(e))
        except requests.exceptions.RequestException as e:
            # Wrap all other requests-specific exceptions in a generic RequestNetworkException.
            raise RequestNetworkException(url, str(e))

        return process_response_with(url, response, allowed_response_codes, disallowed_response_codes)

    @classmethod
    def _process_response(cls, url, response, allowed_response_codes=None, disallowed_response_codes=None):
        """
        Raise a RequestNetworkException if the response code indicates a server-side failure, or behavior
        so unpredictable that we can't continue.

        :param allowed_response_codes If passed, then only the responses with http status codes in this
            list are processed.  The rest generate BadResponseExceptions.
        :param disallowed_response_codes The values passed are added to 5xx, as http status codes that
            would generate BadResponseExceptions.
        """
        ...
        if allowed_response_codes and (code in allowed_response_codes or series in allowed_response_codes):
            # The code or series has been explicitly allowed. Allow the request to be processed.
            return response
        ...
        elif (allowed_response_codes and not (code in allowed_response_codes or series in allowed_response_codes)):
            error_message = status_code_not_in_allowed

        if error_message:
            raise BadResponseException(url, error_message % code, status_code=code,
