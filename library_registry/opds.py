...
class OPDSCatalog:
    ...
    This document may stand on its own, or be contained within another OPDS 2 Catalog Document in
    a collection with the "catalogs" role.

    Within the "catalogs" role, metadata and the navigation collection role have the same semantics
    as in the overall OPDS 2 Catalog spec.
    """
    ...
    def __init__(self, _db, title, url, libraries, annotator=None, live=True, url_for=None):
        ...
        #
        # To save time, omit service area information from large feeds which we know won't use it.
        include_logos = include_service_areas = not (self._feed_is_large(_db, libraries))
        ...
        for library in libraries:
            ...
            self.catalog["catalogs"].append(
                self.library_catalog(*library, url_for=url_for, include_logo=include_logos,
    ...
    @classmethod
    def library_catalog(
        cls, library, distance=None, include_private_information=False, include_logo=True,
        url_for=None, web_client_uri_template=None, include_service_area=False
    ):
        """
        Create an OPDS catalog for a library.

        :param distance: The distance, in meters, from the client's current location (if known)
            to the edge of this library's service area.

        :param include_private_information: If this is True, the consumer of this OPDS catalog
            is expected to be the library whose catalog it is. Private information such as the
            point of contact for integration problems will be included, where it normally wouldn't be.

        :param include_service_area: If this is True, the consumer of this OPDS catalog will be
            using information about the library's service area.
            TODO: This can be removed once we stop using the endpoints that just give a huge
            list of libraries.
        """
        ...
        metadata = {
            ...
            "title": library.name,
            "modified": modified,
            "updated": modified,        # For backwards compatibility with earlier clients.
        }
        ...
        if include_service_area:
            ...
            for code in library.types:
                subjects.append(
                    {"code": code, "name": LibraryType.NAME_FOR_CODE[code], "scheme": LibraryType.SCHEME_URI}
                )
    ...
    def _feed_is_large(cls, _db, libraries):
        ...
        if isinstance(libraries, Query):
            size = libraries.count()        # This is a SQLAlchemy query.
        else:
            size = len(libraries)           # This is something like a normal Python list.
