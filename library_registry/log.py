...
class StringFormatter(logging.Formatter):
    ...
    In Python 2, this means a UTF-8 bytestring. In Python 3, it means a Unicode string.
    """
    def format(self, record):
...
class LogConfiguration:
    ...
    def initialize(cls, _db, testing=False):
        ...
        :param _db: A database connection. If this is None, the default logging configuration will be used.
        ...
        for logger in ('sqlalchemy.engine', 'elasticsearch', 'requests.packages.urllib3.connectionpool'):
            logging.getLogger(logger).setLevel(database_log_level)

        # These loggers can cause infinite loops if they're set to DEBUG, because their log is
        # triggered during the process of logging something to Loggly. These loggers will never
        # have their log level set lower than WARN.
    ...
    def from_configuration(cls, _db, testing=False):
        ...
        :param _db: A database connection. If None, the default logging policy will be used.

        :param testing: A boolean indicating whether a unit test is happening right now. If True,
            the database configuration will be ignored in favor of a known test-friendly policy.
            (It's okay to pass in False during a test *of this method*.)

        :return: A 3-tuple (internal_log_level, database_log_level, handlers). `internal_log_level`
            is the log level to be used for most log messages. `database_log_level` is the log level
            to be applied to the loggers for the database connector and other verbose third-party
            libraries. `handlers` is a list of Handler objects that will be associated with the
            top-level logger.
        """

        # Establish defaults, in case the db is not initialized, or is initialized but logging isn't configured.
        (internal_log_level, internal_log_format, database_log_level, message_template) = cls._defaults(testing)
    ...
    def loggly_handler(cls, externalintegration):
        ...
        try:
            ...
            raise CannotLoadConfiguration("Cannot interpolate token %s into loggly URL %s" % (token, url))
    ...
    def _interpolate_loggly_url(cls, url, token):
        ...
        return url  # Assume the token is already in the URL.
