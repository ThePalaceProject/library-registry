...
class Configuration:
    ...
    # An HTML snippet describing the terms of service for library registration. It's better if this
    # is a short snippet of text with a link, rather than the actual text of the terms of service.
    REGISTRATION_TERMS_OF_SERVICE_HTML = "registration_terms_of_service_html"

    # Email sent by the library registry will be 'from' this address, and receipients will be invited to
    # contact this address if they have problems.
    REGISTRY_CONTACT_EMAIL = "registry_contact_email"

    # If the registry provides access to a web-based client, it can specify the URL with this setting.
    # The URL must be templated and contain a `{uuid}` expression, to provide the web URL for a specific library.
    WEB_CLIENT_URL = "web_client_url"

    # If a library references a place that's not explicitly in any particular nation, we assume that
    # they're talking about this nation.
    DEFAULT_NATION_ABBREVIATION = "default_nation_abbreviation"

    # For performance reasons, a registry may want to omit certain pieces of information from large feeds.
    # This sitewide setting controls how big a feed must be to be considered 'large'.
    ...
    def database_url(cls, test=False):
        ...
        if not url:
            msg = f"Database URL was not defined in env variable ({environment_variable}) or configuration file."
    ...
    def vendor_id(cls, _db):
        ...
        integration = ExternalIntegration.lookup(_db, ExternalIntegration.ADOBE_VENDOR_ID,
